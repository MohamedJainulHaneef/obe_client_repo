...
function Rsmatrix() {
        ...
        const fetchCourseCodes = async () => {
            if (activeSem) {
                try {
                    ...
                    // if(response){
                    //     const rsm = await axios.get(`${apiUrl}/api/rsmatrix/${response.data.course_code}`)
                    //     if(response.data.course_code === rsm.data.course_code){
                    //         setStatus('Completed')
                    //     }
                    //     setStatus('Pending')
                    // }
    ...
    //         if (selectedCourse) {
    //             try {
    //                 const response = await axios.get(`${apiUrl}/api/rsmatrix/${selectedCourse.course_code}`);
    ...
    //                     if(response.data.course_code === selectedCourse.course_code){
    //                         setStatus('Completed')
    //                     }
    //                 }
    //             } catch (err) {
                    ...
                    {courseDetails.length > 0 ? (
                        courseDetails.map((item, index) => (
                            ...
                            >
                                ...
                                {/* <p style={{ color: status === 'Completed' ? 'green' : 'red' }}>{status}</p> */}
                            </div>
                        ))
                    ) : (
