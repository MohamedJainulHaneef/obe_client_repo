...
function ObeReport() {
 ...
 const downloadpdf = async () => {
  ...
  const deptIds = Object.keys(attainmentSpecData); // Used for filenames
  ...
  const cloneAndFixLogo = (node, isSecond = false) => {
    ...
    if (header) header.innerHTML = ""; // Clear existing header content
  ...
  for (let i = 0; i < allDeptSections.length; i += 2) {
    ...
    if (canvas2) {
      ...
      while (position < canvas2.height) {
        ...
        canvasSlice.width = canvas2.width;
    ...
    // Save with deptId from attainmentSpecData
    const deptId = deptIds[i / 2]; // Make sure order matches `.pro-a4` elements
    pdf.save(`${deptId}.pdf`);
  }
};
          ...
          <React.Fragment key={deptId}>
            ...
            <div className="pro-a4">
              ...
              <div className="pro-info">
                ...
                <p><strong>Period of Study :</strong> {/* Add if available */}</p>
              </div>
              {deptData?.overall ? (
                <table className="pro-table">
                  <thead>
